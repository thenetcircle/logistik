...
class HandlerConfEntity(env.dbman.Model):
    ...
    enabled = env.dbman.Column(
        env.dbman.Boolean(), unique=False, nullable=False, server_default="false"
    )
    retired = env.dbman.Column(
        env.dbman.Boolean(), unique=False, nullable=False, server_default="false"
    )
    ...
    version = env.dbman.Column(
        env.dbman.String(128), unique=False, nullable=False, server_default=""
    )
    path = env.dbman.Column(env.dbman.String(128), unique=False, nullable=True)
    node = env.dbman.Column(
        env.dbman.Integer(), unique=False, nullable=False, server_default="0"
    )
    method = env.dbman.Column(env.dbman.String(128), unique=False, nullable=True)
    model_type = env.dbman.Column(
        env.dbman.String(128),
        unique=False,
        nullable=False,
        server_default=ModelTypes.MODEL,
    )
    retries = env.dbman.Column(
        env.dbman.Integer(), unique=False, nullable=False, server_default="1"
    )
    timeout = env.dbman.Column(
        env.dbman.Integer(), unique=False, nullable=False, server_default="0"
    )
    ...
    event_display_name = env.dbman.Column(
        env.dbman.String(128), unique=False, nullable=False, server_default="event"
    )
    startup = env.dbman.Column(env.dbman.DateTime(), unique=False, nullable=True)
    traffic = env.dbman.Column(
        env.dbman.Float(), unique=False, nullable=False, server_default="0.1"
    )
    reader_type = env.dbman.Column(
        env.dbman.String(), unique=False, nullable=False, server_default="kafka"
    )
    reader_endpoint = env.dbman.Column(env.dbman.String(), unique=False, nullable=True)
    consul_service_id = env.dbman.Column(
        env.dbman.String(), unique=False, nullable=True
    )
    ...
    def update(self, handler_conf: HandlerConf):
        ...
        self.event_display_name = (
            handler_conf.event_display_name or self.event_display_name
        )
        ...
        self.consul_service_id = (
            handler_conf.consul_service_id or self.consul_service_id
        )
