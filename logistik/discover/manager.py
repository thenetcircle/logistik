...
class DiscoveryService(BaseDiscoveryService):
    ...
    def __init__(self, env: GNEnvironment):
        ...
        self.tag = env.config.get(
            ConfigKeys.TAG, domain=ConfigKeys.DISCOVERY, default="logistik"
        )
        self.interval = env.config.get(
            ConfigKeys.INTERVAL, domain=ConfigKeys.DISCOVERY, default=30
        )
    ...
    def poll_services(self):
        ...
        for name, metadata in data.items():
            ...
            for service in services:
                ...
                if hostname is None:
                    self.logger.warning(
                        f'hostname is not specified in tags for "{service_id}", ignoring service'
                    )
                    continue
    ...
    def enable_handler(
        ...
        handler_conf = self.create_or_update_handler(
    ...
    def create_or_update_handler(
        self, host, port, service_id, name, node, hostname, c_id, tags: dict
    ):
        """
        ...
        return self._create_new_handler(
            service_id, node, name, hostname, port, host, c_id, tags
        )
    ...
    def _update_existing_handler(
        ...
        """
        if handler.enabled:
            return self.env.db.update_consul_service_id_and_group_id(
                handler, c_id, tags
            )

        self.logger.info(
            'registering updated handler "{}": address "{}", port "{}", id: "{}"'.format(
    ...
    def _create_new_handler(
        self, service_id, node, name, hostname, port, host, c_id, tags: dict
    ) -> HandlerConf:
        """
        ...
        """
        self.logger.info(
            'registering new handler "{}": address "{}", port "{}", id: "{}"'.format(
                name, host, port, service_id
            )
        )

        handler = self._create_handler(
            HandlerConf(), service_id, node, name, hostname, port, host, c_id, tags
        )
    ...
    def _create_handler(
        ...
        handler.group_id = (
            tags.get(ServiceTags.GROUP_ID, None) or service_id.split("-")[0]
        )
        handler.reader_endpoint = (
            tags.get("readerendpoint", None) or handler.reader_endpoint
        )
    ...
    def run(self):
        while True:
            try:
                ...
                self.logger.error(
                    "could not poll service, sleeping for 3 seconds: {}".format(str(e))
                )
