...
def api_response(
    code, data: Union[dict, List[dict], str] = None, message: Union[dict, str] = None
):
    if data is None:
        data = dict()
    if message is None:
    ...
    return jsonify({"status_code": code, "data": data, "message": message})
...
def list_models():
    ...
    return api_response(200, [format_handler(handler) for handler in handlers])
...
def login():
    ...
    callback_url = environ.env.config.get(
        ConfigKeys.CALLBACK_URL, domain=ConfigKeys.WEB, default=root_url
    )
...
def get_graph():
    handlers = environ.env.db.get_all_handlers()
    node_id_enabled = {
        HandlerConf.to_node_id(
            h.service_id, h.hostname, h.model_type, h.node
        ): h.enabled
        for h in handlers
    }
                ...
                "children": [
                    {
                        ...
                        "label": "{}-{}-{}".format(
                            handler.hostname, handler.model_type, handler.node
                        ),
                    }
                    for handler in handlers
                    if handler.service_id == service_id
                ],
            }
    ...
    for handler in data["children"]:
        node = {"id": handler["id"], "label": handler["label"]}
        ...
        for model in handler["children"]:
            m_node = {"id": model["id"], "label": model["label"]}
            ...
            nodes.append(m_node)
            edges.append(
                {"from": node["id"], "to": m_node["id"], "label": str(model["value"])}
            )
...
def index():
    floating_menu = str(
        environ.env.config.get(ConfigKeys.USE_FLOATING_MENU, domain=ConfigKeys.WEB)
    )
    ...
    return render_template(
        ...
        config={
            "ROOT_URL": environ.env.config.get(
                ConfigKeys.ROOT_URL, domain=ConfigKeys.WEB
            ),
